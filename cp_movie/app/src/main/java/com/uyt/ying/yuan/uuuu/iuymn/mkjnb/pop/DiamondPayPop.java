...
/*                SharePreferencesUtil.putString(MyApplication.getInstance(),"userInfo",result);//储存用户信息
    ...
    private void initData() {
        ...
        if(customType==1){
             ...
             */
            if(useDays.equals("0")){
                mount_amount_tv.setText(String.format("%s钻石/永久",medalConditionsBean.getBuyZhuanShi()));
            }else {
                if(medalConditionsBean!=null){
                    mount_amount_tv.setText(String.format("%s钻石/%s天",medalConditionsBean.getBuyZhuanShi(), useDays));
                }
            }
        }
