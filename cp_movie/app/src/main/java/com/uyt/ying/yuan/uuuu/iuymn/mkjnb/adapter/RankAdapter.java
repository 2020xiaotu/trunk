...
//                rank_level_iv.setImageResource(Utils.getLevelDrawble(recordsBean.getGrade()+1));//设置等级icon
                ...
                if(recordsBean.getNickname().equals("等待上榜")){
                    changci_tv.setText("场次: 0中0");//设置中奖场次
                }else {
                    changci_tv.setText("场次: "+ tzNum +"中"+ zjNum);//设置中奖场次
                }
                if(zjNum==0){
                    win_rate_tv.setText("胜率： 0.00%");
                }else {
                    win_rate_tv.setText("胜率: "+new BigDecimal(Double.parseDouble(zjNum+"")/Double.parseDouble(tzNum+"")*100).setScale(2,BigDecimal.ROUND_HALF_DOWN)+"%");//设置中奖比例
                }
                break;
            case 5:
                baseViewHolder.setText(R.id.rank_num_tv,(adapterPosition+3)+"");//排名
