        ...
        if (!isHeaderView(position) && !isFooterView(position)) {
            ...
            if(mineDealMedol.getWhat().equals("1")){//全部类型
                if(mineDealMedol.getType()>100){
                    ...
                    myHolder.amount.setText("-"+ mineDealMedol.getAmout().setScale(2,BigDecimal.ROUND_DOWN)+"元");

                }else{
                    myHolder.amount.setTextColor(Color.RED);
                    BigDecimal amout = mineDealMedol.getAmout();
                    if(amout.compareTo(BigDecimal.ZERO)==-1){
                        myHolder.amount.setText( amout.setScale(2,BigDecimal.ROUND_DOWN)+"元");
                    }else {
...
//                    myHolder.type.setText("已打款");
                ...
                if(mineDealMedol.getType()==0l){
                    ...
                    myHolder.name.setText("微信");//type为1, 设置为 "微信"
                }else if(mineDealMedol.getType()==2l){
                    myHolder.name.setText("支付宝");//type为1, 设置为 "支付宝"
                } else if(mineDealMedol.getType()==3l){
                    if(mineDealMedol.getAmout().compareTo(BigDecimal.ZERO)==-1){
                        myHolder.name.setText(Utils.getString(R.string.rengongkoukuangyue));//金额小于0,
                    }
                    else{
                        myHolder.name.setText(Utils.getString(R.string.rengongrukuangyue));
                    }
                }else if(mineDealMedol.getType()==4l){
                    myHolder.name.setText(Utils.getString(R.string.dailichongzhi));
                }
                //类型/充值类型 0银行 1微信 2支付宝 3后台充值-余额 4代理充值(代理佣金余额充值给下级) 8(price<0 人工扣款-佣金  price>0 人工入款-佣金)后台充值-佣金 9百乐汇 else 在线充值
                    ...
                    if(mineDealMedol.getAmout().compareTo(BigDecimal.ZERO)==-1){
                        myHolder.name.setText(R.string.rengongkoukuangyongjin);//金额小于0,
                    }else{
                        myHolder.name.setText(Utils.getString(R.string.rengongrukuanyongjin));//金额小于0,
                    }
                } else if(mineDealMedol.getType()==9l){
                    myHolder.name.setText("百汇付");
                }else{
                    myHolder.name.setText(Utils.getString(R.string.zaixianchongzhi));
                }
                if(mineDealMedol.getStatus().equals("0")){
                    myHolder.type.setText(Utils.getString(R.string.chongzhizhong));
                    myHolder.type.setTextColor(Color.parseColor("#238BFF"));
                    BigDecimal amout = mineDealMedol.getAmout();
                    if(amout.compareTo(BigDecimal.ZERO)==-1){
                        myHolder.amount.setText(mineDealMedol.getAmout()+"元");
                    }else{
                        myHolder.amount.setText("+"+mineDealMedol.getAmout()+"元");
                    }
                    myHolder.amount.setTextColor(Color.parseColor("#238BFF"));
                }else if(mineDealMedol.getStatus().equals("1")){
                    myHolder.type.setText(Utils.getString(R.string.chongzhichenggong));
                    myHolder.type.setTextColor(Color.parseColor("#FF4540"));
                    BigDecimal amout = mineDealMedol.getAmout();
                    if(amout.compareTo(BigDecimal.ZERO)==-1){
                        myHolder.amount.setText(mineDealMedol.getAmout()+"元");
                    ...
                    String text = String.format("充值失败(%s)", mineDealMedol.getRemark());
