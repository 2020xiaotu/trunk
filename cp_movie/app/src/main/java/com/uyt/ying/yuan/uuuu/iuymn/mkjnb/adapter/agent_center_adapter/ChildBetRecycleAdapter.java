    ...
    public void onBindViewHolder(@NonNull ChildBetRecycleAdapter.MyHolder myHolder, int position) {
        if (!isHeaderView(position) && !isFooterView(position)) {
            ...
            textColors=myHolder.name.getTextColors();
             myHolder.count.setText("第"+childBetModelArrayList.get(position).getLotteryqishu()+"期");
        if(childBetModelArrayList.get(position).getRemark().equals("中奖")){
            myHolder.status.setText("已中奖");
            myHolder.amount.setText(childBetModelArrayList.get(position).getBonus().setScale(2,BigDecimal.ROUND_DOWN)+"元");//中奖金额
            myHolder.status.setTextColor(Color.RED);
            myHolder.amount.setTextColor(Color.RED);
        }else if(childBetModelArrayList.get(position).getRemark().equals("未中")){
            myHolder.status.setText("未中奖");
            myHolder.amount.setText(childBetModelArrayList.get(position).getAmount().setScale(2,BigDecimal.ROUND_DOWN)+"元");//投注金额
            myHolder.status.setTextColor(textColors);
            myHolder.amount.setTextColor(textColors);

        }else {
            myHolder.status.setText(Utils.getString(R.string.dengdaikaijiang));
            myHolder.amount.setText(childBetModelArrayList.get(position).getAmount().setScale(2,BigDecimal.ROUND_DOWN)+"元");//投注金额
        }
