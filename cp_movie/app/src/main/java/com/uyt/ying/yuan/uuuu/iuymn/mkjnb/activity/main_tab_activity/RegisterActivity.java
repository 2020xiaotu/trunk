...
public class RegisterActivity extends BaseActivity /*implements View.OnClickListener*/ {
    ...
    private void showView() {
        ...
        if(yqm.equals("0")){
            invite_code_etv.setHint("请输入邀请码(必填)");
        }else {
            invite_code_etv.setHint("请输入邀请码(选填)");
        }
        ...
        if(BuildConfig.DEBUG){
            register_version_name_tv.setText(String.format("测试版 版本号:%s",BuildConfig.VERSION_NAME));
        }else {
            register_version_name_tv.setText(String.format("版本号:%s",BuildConfig.VERSION_NAME));
        }
    }
    /**
     * 弹出滑动pop
