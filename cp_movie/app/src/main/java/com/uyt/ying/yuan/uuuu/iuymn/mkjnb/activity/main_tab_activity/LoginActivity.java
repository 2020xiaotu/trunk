        ...
        HttpApiUtils.CPnormalRequest(this, null, RequestUtil.VISITOR_SHOW, data, new HttpApiUtils.OnRequestLintener() {
            ...
            public void onSuccess(String result, Headers headers) {
                ...
                if(visitor_login_close.equals("0")&&visitorLogin.equals("1")){
                    toolbar_right_tv.setText("游客登录");
                }else {
                    toolbar_right_tv.setText("注册");
                }
...
//                    initSlideWebViewPop(false);
            ...
            case R.id.toolbar_right_tv:
                if(Utils.isNotFastClick()){
                    ...
                    if(toolbar_right_tv.getText().toString().equals("注册")){
    ...
    Runnable runnable =new Runnable() {
        ...
        public void run() {
            if(singleLogin){
                ...
                //触发后台单点登录时跳而来
                if(flag==1){
                    topTv.setText("Token无效");
                    buttomTv.setText("请重新登录");
                }
                //触发融云单点登录时跳转而来
                else {
                    topTv.setText("当前账号在另一地点登录");
                    buttomTv.setText("您已被强制下线");
                }
...
/*        if (!Utils.checkAccount(account)){
            showtoast("请输入字母开头6-11位字母加数字的账号");
            return false;
        } else  if (!Utils.checkPsw(password)){
            showtoast("请输入6-16位字母加数字的密码");
            return false;
        } */
        else if(account.length()<6){
            showToast("账号不符合规范,请重新输入");
            return false;
        }else  if (!Utils.checkPsw(password)){
            showToast("请输入6-16位字母加数字的密码");
            return false;
    ...
    Runnable countDownRunnable = new Runnable() {
        ...
        public void run() {
            countDownTime--;
            if(countDownTime>=1){
                get_phone_code_tv.setText(countDownTime+"s后重发");
...
//                        String chatRoomId = SharePreferencesUtil.getString(LoginActivity.this, "chatRoomId", "");
                ...
                case 2:
                    if(StringMyUtil.isEmptyString(tencent_randstr)&&appSlidingCheck.equals("0")){
                        ...
                        //请求登录接口
//                        ProgressDialogUtil.show(LoginActivity.this,"登录中",false);
...
//                        PopupWindowUtil.disMiss(LoginActivity.this);
    ...
    private void flip( boolean scale, boolean reverse) {
        ...
        if(phone_login_constraintLayout.getVisibility() == View.VISIBLE){
            toolbar_title_tv.setText("手机号登录");
        }else {
            toolbar_title_tv.setText("登录");
        }
