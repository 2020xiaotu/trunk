...
public class LongDragonBetNoteRecycleAdapter extends CommonAdapter<LongDragonBetNoteRecycleAdapter.MyHolder,MineBetModel> {
    ...
    public void handleViewHolder(MyHolder commonHolder, int position) {
        ...
        commonHolder.qishu.setText("第"+list.get(position).getLotteryqishu()+"期");
        if(list.get(position).getRemark().equals("中奖")){
            commonHolder.remark.setText("已中奖");
            commonHolder.amount.setText(list.get(position).getBonus().setScale(2,BigDecimal.ROUND_DOWN)+"元");//中奖金额
            commonHolder.remark.setTextColor(Color.RED);
            commonHolder.amount.setTextColor(Color.RED);

        }else if(list.get(position).getRemark().equals("未中")){
            commonHolder.remark.setText("未中奖");
            commonHolder.amount.setText(list.get(position).getAmount().setScale(2,BigDecimal.ROUND_DOWN)+"元");//投注金额
            commonHolder.remark.setTextColor(Color.parseColor("#868686"));
            commonHolder.amount.setTextColor(Color.parseColor("#868686"));

        }else if (list.get(position).getRemark().equals(Utils.getString(R.string.cedan))){
            commonHolder.remark.setText(Utils.getString(R.string.yicedan));
            commonHolder.amount.setText(list.get(position).getAmount().setScale(2,BigDecimal.ROUND_DOWN)+"元");//投注金额
            commonHolder.remark.setTextColor(Color.parseColor("#868686"));
            commonHolder.amount.setTextColor(Color.parseColor("#868686"));
        }
        else{
            commonHolder.remark.setText(Utils.getString(R.string.dnegdaikaijiang));
            commonHolder.amount.setText(list.get(position).getAmount().setScale(2,BigDecimal.ROUND_DOWN)+"元");//投注金额
            commonHolder.remark.setTextColor(Color.parseColor("#868686"));
            commonHolder.amount.setTextColor(Color.parseColor("#868686"));
        }
