...
public class BetNumUtil {
    ...
    public static int zhiXuanFuShi(ArrayList<SscOfiicialBetMedol> officialSelectorList,String typeOneName){
        ...
        for (int j = 0; j < officialSelectorList.size(); j++) {
            ...
            if(name.equals("万位")){
                wanwei++;
            }else if(name.equals("千位")){
                qianwei++;
            }else if(name.equals("百位")){
                baiwei++;
            }else if(name.equals("十位")){
                shiwei++;
            }else {
                gewei++;
            }
       ...
       int resultCount=0;
        //||typeOneName.equals("后二")||typeOneName.equals("前三")||typeOneName.equals("中三")||typeOneName.equals("后三")
        if(typeOneName.equals(Utils.getString(R.string.qianer))){
                resultCount=wanwei*qianwei;
        }else if(typeOneName.equals(Utils.getString(R.string.houer))){
                resultCount=shiwei*gewei;
        }else if(typeOneName.equals(Utils.getString(R.string.qiansan))){
            resultCount=wanwei*qianwei*baiwei;
        }else if(typeOneName.equals(Utils.getString(R.string.zhongsan))){
            resultCount=qianwei*baiwei*shiwei;
        }else if(typeOneName.equals(Utils.getString(R.string.housan))){
            resultCount=baiwei*shiwei*gewei;
        } else if(typeOneName.equals(Utils.getString(R.string.sixing))){
                resultCount=qianwei*baiwei*shiwei*gewei;
        }else if(typeOneName.equals(Utils.getString(R.string.wuxingxing))){
...
/*                                    oneBetArray.sort(new Comparator<Integer>() {
    ...
    public  static int combinZuXuan12_4_60_20WithSectionArray(ArrayList<SscOfiicialBetMedol> officialSelectorList,int sersionTwoBallCount,int sersionCount ){
        ...
        if(sersionCount!=selectedMap.size()){
            ...
            int resultCount=0;
                listOne = selectedMap.get(Utils.getString(R.string.erchonghaowei));//n重号位
            if(listOne==null){
                listOne = selectedMap.get(Utils.getString(R.string.sanchonghaowei));//n重号位
            }
            listTwo = selectedMap.get(Utils.getString(R.string.danhaowei));
    ...
    public  static int combinZuXuan30WithSectionArray(ArrayList<SscOfiicialBetMedol> officialSelectorList,int sersionTwoBallCount,int sersionCount ){
        ...
        int resultCount=0;
        listOne = selectedMap.get(Utils.getString(R.string.danhaowei));//单位号 至少选择1个
        listTwo = selectedMap.get(Utils.getString(R.string.erchonghaowei));//二重号位 至少选择2个
    ...
    public  static int combinZuXuan10_5WithSectionArray(ArrayList<SscOfiicialBetMedol> officialSelectorList,String typeThreeName,int sersionTwoBallCount,int sersionCount ){
        ...
        int resultCount=0;
        if(typeThreeName.equals(Utils.getString(R.string.zuxuan10))){
            listOne = selectedMap.get(Utils.getString(R.string.erchonghaowei));
            listTwo = selectedMap.get(Utils.getString(R.string.sanchonghaowei));
        }else if(typeThreeName.equals(Utils.getString(R.string.zuxuan5))){
            listOne = selectedMap.get(Utils.getString(R.string.danhaowei));
            listTwo = selectedMap.get(Utils.getString(R.string.sichonghaowei));
        }
