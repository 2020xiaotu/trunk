...
//    private String[] titles = new String[]{"全部","已中奖","未中奖","等待开奖"};//tab Title数组
    ...
    public void initView() {
        ...
        titles.add("全部");
        titles.add(getString(R.string.yizhongjiang));
        titles.add(getString(R.string.weizhongjiang));
        titles.add(getString(R.string.dengdaikaijiang));
...
//        FragmentAdapter fragmentAdapter = new FragmentAdapter(getSupportFragmentManager(), titles, fragmentArrayList);
        ...
        Utils.docking(Utils.getNavigateListMap(0), RequestUtil.REQUEST_01dhnew, -1, new DockingUtil.ILoaderListener() {//彩票分类
            ...
            public void success(String content) {
                ...
                ChildBetAndDealAllPop child = new ChildBetAndDealAllPop(getString(R.string.quanbucaipiao), "", "", "");
    ...
    public void onTabSelected(TabLayout.Tab tab) {
        ...
        else if(tab.getText().equals(getString(R.string.yizhongjiang))){
            pageNum=1;
            status=getString(R.string.zhongjiang);
            initTime();
            EventBus.getDefault().postSticky(new ChildBetEvenModel(pageNum,pageSize,status,startDate,endDate,initEdit(),game,typoeId,false,false));
        }
        else if(tab.getText().equals(getString(R.string.weizhongjiang))){
            pageNum=1;
            status=getString(R.string.weizhong);
            initTime();
            EventBus.getDefault().postSticky(new ChildBetEvenModel(pageNum,pageSize,status,startDate,endDate,initEdit(),game,typoeId,false,false));
        }
        else if(tab.getText().equals(getString(R.string.dengdaikaijiang))){
            pageNum=1;
            status=getString(R.string.dengdaikaijiang);
            initTime();
            EventBus.getDefault().postSticky(new ChildBetEvenModel(pageNum,pageSize,status,startDate,endDate,initEdit(),game,typoeId,false,false));
        }
    }
