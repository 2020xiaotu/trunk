...
//        }
    ...
    private void initManualTipPop() {
        if(manualRechargeTipPop==null){
            manualRechargeTipPop = new ManualRechargeTipPop(RechargeActivity.this,true,checkRemark(currentAisleEntity.getRemark(),manualTip),"代充提示");
                ...
                if(rechargeSkipTipPop!=null){
                    ...
                    switch (skipType){
                        case QQ:
                            ...
                            break;
                        case WE_CHAT:
                            startOtherActivity("com.tencent.mm","微信");
                            break;
                        case ALI_PAY:
                            startOtherActivity("com.eg.android.AlipayGphone","支付宝");
                            break;
                        default:
                            break;
                    }
        ...
        switch (v.getId()){
            ...
            case R.id.recharge_marquee_view:
                if(noticeTipPop==null){
                    noticeTipPop = new ManualRechargeTipPop(this,true,noticeContent,"重要通知");
...
//                            }
        ...
        if(currentAisleEntity.getType().equals("16")){
            if(new BigDecimal(ustdNum).compareTo(BigDecimal.ZERO)==0){
                ...
                if(parseFloat < mix || parseFloat > max){
                    showToast("货币数量超出区间,请重新输入");
                    return  false;
                }
            }
            if(StringMyUtil.isEmptyString(ustdId)){
                showToast("请输入区块链交易ID");
            }
        }else {
    ...
    private void initRechargeTipStr() {
        ...
        if(type.equals("0")){
            //公司入款
            tipContent = checkRemark(remark,"请转账到以上收款账户,转账成功后工作人员将第一时间为您入款");
        }else if(type.equals("1")){
                //微信
                tipContent = checkRemark(remark, "1.打开微信扫描二维码或转账到银行卡进行充值；2.可将二维码保存至相册，用微信识别相册中的二维码，每次充值时请在本页面获取最新二维码。");
            }else if(type.equals("2")){
                //支付宝
                 tipContent = checkRemark(remark, "1.打开支付宝扫描二维码或转账到银行卡进行充值；2.可将二维码保存至相册，用支付宝识别相册中的二维码，每次充值时请在本页面获取最新二维码。");
            }else if(type.equals("3")){
                //qq钱包
                 tipContent = checkRemark(remark, "请转账到以上收款账户,转账成功后工作人员将第一时间为您入款");
            }else if(type.equals("16")){
            //USDT
            tipContent =checkRemark(remark,"温馨提示:\n使用数字货币存取款，避免银行流水过大被风控，大额娱乐首选");
          }else {
            tipContent =checkRemark(remark,manualTip);
            }
        Utils.webViewLoadColorStr(tipContent,recharge_tip_wbv,"#FF6D23");
    ...
    private String checkRemark(String remark, String s) {
        ...
        return tipContent.equals("人工充值")?manualTip:tipContent;
    }
...
//        }
    ...
    private void startOtherActivity(String pakegeName,String appName) {
        try {
            ...
            e.printStackTrace();
            showToast("检查到您的手机未安装"+appName+"请安装后再试");
        }
    }
}