    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder viewHolder, final int position) {
        ...
        switch (itemViewType) {
            ...
            case 1:
                ...
                itemViewHolder.betRebate.setText("赔 "+sscModel.getRebate());
...
//        if(position ==0 ||position ==5||position ==12||position==15||position==28||position==32||position==37) {
        if(sscMedolList.get(0).getBetType().equals(Utils.getString(R.string.zonghe_longhuhe))){
            if(position ==0 ||position ==8||position ==13||position==18||position==23||position==28) {
                return 0;
            } else  {
                return 1;
            }
        }else if(sscMedolList.get(0).getBetType().equals(Utils.getString(R.string.diyiqiu))){
