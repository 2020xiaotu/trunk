...
public class DiamondAdapter extends BaseQuickAdapter<DiamondEntity.GainZhuanShiConditionsListBean, BaseViewHolder> {
    ...
    protected void convert(@NotNull BaseViewHolder baseViewHolder, DiamondEntity.GainZhuanShiConditionsListBean gainZhuanShiConditionsListBean) {
        ...
        boolean isGet=false;
        if (type.equals("0")){
            item_title_tv.setText(String.format("邀请%s人礼包",gainZhuanShiConditionsListBean.getPrice()));
        }else if(type.equals("1")){
            item_title_tv.setText(String.format("每日充值%s元礼包",gainZhuanShiConditionsListBean.getPrice()));
        }else if(type.equals("2")){
            item_title_tv.setText(String.format("每日打码%s元礼包",gainZhuanShiConditionsListBean.getPrice()));
        }else if(type.equals("3")){
            item_title_tv.setText(String.format("每日送礼%s元礼包",gainZhuanShiConditionsListBean.getPrice()));
        }
        String userGainSuccess = gainZhuanShiConditionsListBean.getUserGainSuccess();//用户是否获取(0否;1是)
        if(userGainSuccess.equals("0")){
            item_status_tv.setText("未达到");
            item_status_tv.setTextColor(Color.parseColor("#999999"));
            isGet=false;
        }else {
            item_status_tv.setText("已获得");
       ...
       // -------------------------------------------------------------------   child recyclerView start -----------------------
        ...
        if(gainZhuanShiByGift!=null){
            ...
            if(StringMyUtil.isNotEmpty(medalInfo0Id)&& medalInfo0Status.equals("1")){
                ...
                DiamondChildEntity diamondChildEntity = new DiamondChildEntity(medalInfo0Name, medalInfo0Image,  type0Days + "天",isGet);
            ...
            if(StringMyUtil.isNotEmpty(medalInfo1Id)&&medalInfo1Status.equals("1")){
                ...
                DiamondChildEntity diamondChildEntity = new DiamondChildEntity(medalInfo1Name, medalInfo1Image,  type1Days + "天",isGet);
                childList.add(diamondChildEntity);
            }
            ...
            if(StringMyUtil.isNotEmpty(medalInfo2Id)&&medalInfo2Status.equals("1")){
                ...
                DiamondChildEntity diamondChildEntity = new DiamondChildEntity(meda2Info1Name, meda2Info1Image,  type2Days + "天",isGet);
                childList.add(diamondChildEntity);
            }
        }
        if(StringMyUtil.isNotEmpty(gainZhuanShi)&&!gainZhuanShi.equals("0")){
            ...
            DiamondChildEntity diamondChildEntity = new DiamondChildEntity("钻石", "",  gainZhuanShi + "个",isGet);
            childList.add(diamondChildEntity);
        }
