...
//                selecterList.clear();
        ...
        for (int i = 0; i < danRulelist.size(); i++) {
            ...
            if (hunheRb.isChecked()) {//actionBar中选中"混合"按钮
                if (jsonObject.getString("groupname").equals(Utils.getString(R.string.hunhe))) {//groupname为Utils.getString(R.string.hunhe),取出
                ...
                if (jsonObject.getString("groupname").equals(Utils.getString(R.string.bose))) {
                    String name = jsonObject.getString("name");//类型(大小 单双 小单 .....)
                    String odds = jsonObject.getString("odds");//赔率
                    String id = jsonObject.getString("id");//玩法id
                    String groupname = jsonObject.getString("groupname");//玩法类型
                ...
                if (jsonObject.getString("groupname").equals(Utils.getString(R.string.tema))) {
                    String name = jsonObject.getString("name");//类型(大小 单双 小单 .....)
                    String odds = jsonObject.getString("odds");//赔率
                    String id = jsonObject.getString("id");//玩法id
                    ArrayList<String> idList = new ArrayList<>();
...
//                        System.out.println("开奖结果暂停 isWaite");
...
//                countTime = countTime - 1000;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.xingyong_play:
                showToast(Utils.getString(R.string.zhanwugaiyouxiguanfangwanfa));
                break;
            //点击弹出右侧菜单
                    ...
                    *//*
                    ...
                    for (int i = 0; i < selecterList.size(); i++) {
                        ...
                        if (pcDanDanRecycleModel.getGroupname().equals(Utils.getString(R.string.hunhe))) {
                            ...
                            needString += betType + " ";
                        } else if (pcDanDanRecycleModel.getGroupname().equals("波色")) {
                            String betType = pcDanDanRecycleModel.getBetType();
                            needString2 += betType + " ";
                        } else {
                            String betType = pcDanDanRecycleModel.getBetType();
    ...
    private void xingYongRandom() {
        ...
        for (int i = 0; i < selecterList.size(); i++) {
            ...
            if (betType.equals(Utils.getString(R.string.hunhe)) || betType.equals(Utils.getString(R.string.bose)) || betType.equals(Utils.getString(R.string.tema))) {
                xingYongRandom();
            } else {
                break;
            }
