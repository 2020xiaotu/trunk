        ...
        Utils.docking(data, RequestUtil.REQUEST_zz3, 1, new DockingUtil.ILoaderListener() {
            ...
            public void success(String content) {
                ...
                for (int i = 0; i < bonusRecordList.size(); i++) {
                    ...
                    String info ="恭喜"+nickname+"成功邀请会员注册,获得红包"+yqRedPrice+"元。";
...
//                  OpenInviteRedActivity.startAty(InviteAndMakeMoneyActivity.this,redNum,"");
    ...
    private void initRuleList(  HbGameClassModel instance) {
        ...
        ruleModelArrayList.add(new RuleModel("1.每邀请1人成功注册,可获得1个邀请红包,每包金额"+quYueHBMinAmount+"-"+quYueHBMaxAmount+"元。"));
        ruleModelArrayList.add(new RuleModel("2.有效邀请人数每满"+times+"人,即可拆一轮红包，等级要求必须达到VIP"+quYueHBGrade+"才能参与!"));
        ruleModelArrayList.add(new RuleModel("3.打开红包100%获得奖金,最高每包可以抢到"+quYueHBMaxAmount+"元。"));
        ruleModelArrayList.add(new RuleModel("4.相同IP地址、相同设备码均视为同一个人注册,如未真实注册,重复注册,等非法行为均会取消资格。"));
        BigDecimal multiply = new BigDecimal(cpTzFyRate).multiply(new BigDecimal("10")).setScale(2,BigDecimal.ROUND_DOWN);
        ruleModelArrayList.add(new RuleModel(String.format("邀请朋友成为下级，将获得平台给您的返佣，下级投注后，" +
                "您可以获得%s%%独家返佣,(朋友每下注1000元平台返您%s元）！)",cpTzFyRate, multiply)));
    }
    ...
    private void requestInviteRedList( ) {
        HttpApiUtils.CPnormalRequest(this,null, RequestUtil.MAKE_MONEY_RULE, new HashMap<String, Object>(), new HttpApiUtils.OnRequestLintener() {
            ...
            public void onSuccess(String result, Headers headers) {
                ...
                processArrayList.add(new MakeMoneyModel(R.drawable.img1,"已邀请好友",peopleCount+""));
                processArrayList.add(new MakeMoneyModel(R.drawable.img2,"有效邀请人数",effectPeople+""));
/*                processArrayList.add(new MakeMoneyModel(R.drawable.img3,"累计积分",totalInvitePoint+""));
                processArrayList.add(new MakeMoneyModel(R.drawable.img4,"剩余积分",invitePoint+""));*/
                processArrayList.add(new MakeMoneyModel(R.drawable.img5,"昨日收益",redPriceYesterday+"元"));
                processArrayList.add(new MakeMoneyModel(R.drawable.img6,"累计收益",totalPrice+"元"));
    ...
    public void OpenPackPop(String redId, RedStatus redStatus, String qbPrice, RedType redType, String roomId, String failInfo) {
       CusPackPop mCusPackPop = new CusPackPop(redId, this, new LiveFragment(), redStatus, qbPrice, redType, roomId, failInfo);
        ...
        mCusPackPop.setmOnItemClickListener((View v, CusPackPop.CLICKTYPE clicktype) -> {
            switch (clicktype) {
                case OPEN:
//                    ToastUtils.showToast("点击开红包");
                    break;
                case UNLOCK:
//                    ToastUtils.showToast("点击解锁");
                    RouteUtils.start2Share(InviteAndMakeMoneyActivity.this,shareContent+inviteCode);
                    break;
                case UNLOCK_GAMERULES:
//                    ToastUtils.showToast("点击游戏规则");
                    break;
                case UNLOCK_RANK:
//                    ToastUtils.showToast("点击排行榜1");
                    break;
                case PACK_RANK:
//                    ToastUtils.showToast("点击排行榜2");
                    break;
            }
        });
    }
