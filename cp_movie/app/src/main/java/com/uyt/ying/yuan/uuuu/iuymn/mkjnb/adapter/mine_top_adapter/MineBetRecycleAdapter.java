    ...
    public void onBindViewHolder(@NonNull MineBetRecycleAdapter.MyHolder myHolder, int position) {
        if (!isHeaderView(position) && !isFooterView(position)) {
            ...
            long l = Long.parseLong(mineBetModelArrayList.get(position).getCreatedDate());
             ...
             myHolder.qishu.setText("第"+mineBetModelArrayList.get(position).getLotteryqishu()+"期");
        if(mineBetModelArrayList.get(position).getRemark().equals(Utils.getString(R.string.zhongjiang))){
            myHolder.remark.setText(Utils.getString(R.string.yizhongjiang));
            myHolder.amount.setText(mineBetModelArrayList.get(position).getBonus().setScale(2,BigDecimal.ROUND_DOWN)+"元");//中奖金额
            myHolder.remark.setTextColor(Color.RED);
            myHolder.amount.setTextColor(Color.RED);

        }else if(mineBetModelArrayList.get(position).getRemark().equals("未中")){
            myHolder.remark.setText(Utils.getString(R.string.weizhongjiang));
            myHolder.amount.setText(mineBetModelArrayList.get(position).getAmount().setScale(2,BigDecimal.ROUND_DOWN)+"元");//投注金额
            myHolder.remark.setTextColor(Color.parseColor("#868686"));
            myHolder.amount.setTextColor(Color.parseColor("#868686"));

        }else if (mineBetModelArrayList.get(position).getRemark().equals(Utils.getString(R.string.cedan))){
            myHolder.remark.setText(Utils.getString(R.string.yicedan));
            myHolder.amount.setText(mineBetModelArrayList.get(position).getAmount().setScale(2,BigDecimal.ROUND_DOWN)+"元");//投注金额
            myHolder.remark.setTextColor(Color.parseColor("#868686"));
            myHolder.amount.setTextColor(Color.parseColor("#868686"));
        }
        else{
            myHolder.remark.setText(Utils.getString(R.string.dengdaikaijiang));
            myHolder.amount.setText(mineBetModelArrayList.get(position).getAmount().setScale(2,BigDecimal.ROUND_DOWN)+"元");//投注金额
            myHolder.remark.setTextColor(Color.parseColor("#868686"));
            myHolder.amount.setTextColor(Color.parseColor("#868686"));
        }
