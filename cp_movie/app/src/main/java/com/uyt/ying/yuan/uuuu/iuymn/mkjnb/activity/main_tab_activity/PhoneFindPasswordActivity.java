                ...
                if(StringMyUtil.isNotEmpty(nickname)){
                    if(StringMyUtil.isNotEmpty(phone)){
                        if(nicknameStr.length()==11&&Utils.isAllNum(nicknameStr)){
                            ...
                            forget_phone_tv.setText("已绑定手机号"+phone);
                        ...
                        find_password_btn.setText("立即修改");
                    }else {
                         //只有账号 跳转账号找回密码页面
...
//                                forget_phone_tv.setVisibility(View.VISIBLE);
    ...
    Runnable countDownRunnable = new Runnable() {
        ...
        public void run() {
            countDownTime--;
            if(countDownTime>=1){
                send_phone_code_tv.setText(countDownTime+"s后重发");
