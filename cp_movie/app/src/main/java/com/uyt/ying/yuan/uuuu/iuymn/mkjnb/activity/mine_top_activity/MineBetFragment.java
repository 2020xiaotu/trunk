...
public class MineBetFragment extends BaseFragment implements View.OnClickListener {
    ...
    private TextView toBet;//没有数据时的 "去投注"按钮
...
//        game = getIntent().getStringExtra("game");
    ...
    public void getMineBet (MineBetEvenModel mineBetEvenModel){
        ...
        例如: 进入mineBetActivity默认请求的是全部类型的投注记录, 此时remake为""(空), 不对remake做判断处理时,默认会将""(空)作为前后两个fragment数据请求的参数,
              此时"已中奖"fragemnt中的数据预加载为remake为空时的数据(即跟"全部"fragemnt中数据一致),()滑动到 "已中奖" 对应的fragemnt下时, 再重新请求remake为"中奖"的数据,
              请求数据完成后,更换数据会导致闪烁
         */
        ...
        if(tabName.equals("全部")){
            remark="";
        }else if(tabName.equals("已中奖")){
            remark="中奖";
        }else  if(tabName.equals("未中奖")){
            remark="未中";
        }else if(tabName.equals("待开奖")){
            remark="等待开奖";
        }else {
            remark="撤单";
        }
...
//                loadMoreList.clear();
                ...
                for (int i = 0; i < mineBetModelArrayList.size(); i++) {
                    ...
                    if(!mineBetModelArrayList.get(i).getRemark().equals("撤单")){
                        amountAll=amount.add(amountAll);
                        bonusAll=bonus.add(bonusAll);
                    }
                }
                amountPrice.setText("下注:  "+amountAll+"元");
                bonusPrice.setText("中奖:  "+bonusAll+"元");
                mineBetRecycleAdapter.notifyDataSetChanged();
            }
