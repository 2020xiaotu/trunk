...
//        FragmentAdapter fragmentAdapter = new FragmentAdapter(getSupportFragmentManager(), titles, fragmentArrayList);
       ...
       筛选栏 recycleView 点击事件
        ...
        Utils.docking(dataAllType, RequestUtil.REQUEST_01dh, -1, new DockingUtil.ILoaderListener() {
            ...
            public void success(String content) {
                ...
                ChildBetAndDealAllPop allPop = new ChildBetAndDealAllPop("全部类型", "", "", "");
    ...
    public void onTabSelected(TabLayout.Tab tab) {
            ...
            tab切换时,对请求接口的参数进行处理,并发送事件.并将参数发送给订阅者(fragment)
             ...
             (因为TabLayout在initView()中初始化后,会默认选中第一个tab,并发送事件,而此时fragment中尚未订阅该事件,使用普通post会导致接受不到第一次"所有类型"的事件(空指针异常),并且后面在第一次下拉刷新时,参数为空(请求不到数据))
             */
        if (tab.getText().equals("所有类型")) {
        ...
        } else if (tab.getText().equals("提现明细")) {
    ...
    /*
    提现明细status相关初始化
    stutas 为""时,表示选中了"全部类型"按钮
    status为"-1"时,表示选中了"提现失败"按钮
    status为"0"时,表示选中了"申请中"按钮
    status为"1"时,表示选中了"提现成功"按钮
    */
...
充值明细status相关初始化
stutas 为""时,表示选中了"全部类型"按钮
status为"0"时,表示选中了"充值中"按钮
status为"1"时,表示选中了"充值成功"按钮
status为"2"时,表示选中了"充值失败"按钮
*/
